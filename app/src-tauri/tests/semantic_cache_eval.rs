...
fn semantic_threshold_has_zero_false_positives() {
    ...
    let para = [
        (
            ...
            "What is the ranking method in hybrid search?",
        ),
        (
            "How do I add an API key?",
            "Where do I enter my Anthropic API key?",
        ),
        (
            "What does the answer cache do?",
            "Explain how the answer cache works.",
        ),
        (
            "How do I open a vault?",
            "How can I select my notes folder?",
        ),
        ("What is consensus mode?", "Explain how consensus works."),
        (
            "How do I use a local model?",
            "How can I run generation with Ollama locally?",
        ),
    ];
    ...
    let trap = [
        ...
        (
            "Explain the consensus mode.",
            "Explain the embedding model download.",
        ),
        ("How do I open a vault?", "How do I export my notes?"),
        (
            "How do I use a local model?",
            "How do I review my git diff?",
        ),
    ];
    ...
    assert_eq!(
        fp, 0,
        "Anayasa Madde 9: tuzak çiftlerde false-positive OLMAMALI (TH={TH})"
    );
}
