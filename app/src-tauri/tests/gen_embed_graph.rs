...
fn stub_embed_passages_batch_equals_single() {
    ...
    let e = StubEmbedder;
    let texts = vec![
        "alpha passage".to_string(),
        "beta different text".to_string(),
    ];
...
fn graph_build_links_existing_notes() {
    ...
    assert!(
        g.links
            .iter()
            .any(|l| l.source == "a.md" && l.target == "b.md"),
        "a→b linki olmalı: {:?}",
        g.links
    );
    assert!(
        g.nodes.iter().all(|n| !n.dangling),
        "var olan notlar dangling değil"
    );
}
