...
def _resolve_prompt(prompt: str, mode: str) -> str | None:
    if prompt:
        return prompt
...
def cmd_plan(opts: dict) -> int:
    ensure_home()
    task = _resolve_prompt(opts["prompt"], "plan")
    if not task:
...
def cmd_fix(opts: dict) -> int:
    ...
    task = _resolve_prompt(opts["prompt"], "fix")
    if not task:
...
def cmd_ship(opts: dict) -> int:
    ...
    ensure_home()
    task = _resolve_prompt(opts["prompt"], "ship")
    if not task:
...
def main(argv: list[str]) -> int:
    ...
    pos = opts["_pos"]
    if len(pos) == 1 and pos[0].isalpha():
        word = pos[0]
